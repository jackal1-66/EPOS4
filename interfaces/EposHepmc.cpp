...
HepMC3::WriterGZ<HepMC3::WriterAscii> *writerGZ;
#else
HepMC3::WriterAscii *writer;
#endif
...
void openhepmc_(const char *filename) {
  ...
  std::string version_filename = std::string(std::getenv("EPO4")) + std::string("VERSION.txt");
  ...
  if (hepmc_filename.find(prefix) == std::string::npos && hepmc_filename.find("z-") != 0) {
    std::cout << "Custom HepMC filename used" << std::endl;
    prefix = "";
  }
  option_filename = hepmc_filename.substr(hepmc_filename.find_last_of("/") + prefix.length() + 1,std::string::npos);
  ...
  checkStd = option_filename.substr(0, option_filename.find_last_of('.'));
  // Check if the remaining string is equal to "HepMC"
  if (!(strcmp(checkStd.c_str(), "HepMC") == 0)) {
    std::cout << "ASCII HepMC will be created, no compression with checkStd = " << checkStd << std::endl;
    asciiWrite = true;
  }
  // Check if checkStd is an existing FIFO
  std::filesystem::path checkStdPath((std::filesystem::current_path() / checkStd).string());
  if (std::filesystem::is_fifo(checkStdPath)) {
    std::cout << "Found HepMC FIFO" << std::endl;
    checkFifo = true;
  }
  ...
  if(strcmp(filename, "/dev/stdout") == 0 || asciiWrite) {
    ...
    writerGZ = new HepMC3::WriterGZ<HepMC3::WriterAscii>(hepmc_gz_filename, run);
  }
#else
  if (checkFifo) {
    writer = new HepMC3::WriterAscii(checkStd, run);
