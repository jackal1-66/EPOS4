...
c----------------------------------------------------------------------
      subroutine hnbaaa(jcase,ip,iret)  !former hnbaaa156 from epos-yyy
c----------------------------------------------------------------------
...
c      write(ifch,*)'hnb :  uds=',keu,ked,kes,'   E=',pptl(5,ip)
    ...
    !~~~~~~~~~~~~~~~~~~~~~~~~~~ 6 ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      ...
      if(jcase.eq.6)then
        ...
        !transform cluster pt to mass
        tecm=sqrt(tecm**2+c(1)**2+c(2)**2) 
        c(1)=0.
        c(2)=0.
     ...
     &    ((sinh(yrmax)*yrmax-cosh(yrmax)+1d0)/(yrmax**2/2d0)))
        ...
        if(ish.ge.2)
     &  write(ifch,'(a,3f7.2,i8)')'==> cluster energy: '
     &                               ,pptl(5,ip),tecmx,tecm,ip

    !~~~~~~~~~~~~~~~~~~~~~~~~~~ 7 ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

      elseif(jcase.eq.7)then

        ! nothing to do

    !~~~~~~~~~~~~~~~~~~~~~~~~~~ 8 ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

      elseif(jcase.eq.8)then

        ! nothing to do

    !~~~~~~~~~~~~~~~~~~~~~~~~~~ 9 ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

      elseif(jcase.eq.9)then !for droplets from remnants
        ...
        call remn1paramget(6, remnRapScal )
        yco=remnRapScal

        if(yco.gt.0.)then
          if(tecm*yco/sinh(yco).gt.amin)then
            tecm=tecm*yco/sinh(yco)
            !else
            !print*,'TESThnbaa',tecm, tecm*yco/sinh(yco) ,amin
          endif
        endif

    !~~~~~~~~~~~~~~~~~~~~~~~~~~ 3 ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

      elseif(jcase.eq.3)then ! PFE option

        amtc=sqrt(c(5)**2+c(1)**2+c(2)**2)
        rapc=sign(1.d0,c(3))*log( (c(4)+abs(c(3))) / amtc )
        rapca=abs(rapc)
        rapref=0.5*log(engy)
        call pfe3paramget5( yrmax1 , ycoi, ycoj, fecc, taufo )
        yco=ycoi*(1+ycoj*min(1.,rapca/rapref))
        yrmax=yrmax1
        if(yrmax.gt.1d-5)then
          fradflo=sngl(1d0/
     &    ((sinh(yrmax)*yrmax-cosh(yrmax)+1d0)/(yrmax**2/2d0)))
        ...
        if(ish.ge.2)
     &  write(ifch,'(a,3f7.2,i7,i12,i7)')  '==> cluster energy: '
     &                 ,pptl(5,ip),tecmx,tecm,ip,idptl(ip),ityptl(ip)
...
1     continue
 ...
 611    energ=0.
        niter=niter+1
        do i=1,np
          pt2=0.!pcm(1,i)**2+pcm(2,i)**2+amass(i)**2
     ...
     *         , pcm(1,i), pcm(2,i), pcm(3,i), pcm(4,i))  !KW2012 first argu -1 !!!
          do j=1,4
          pe(j)=pe(j)+pcm(j,i)
          enddo
        enddo
     ...
     *         , pcm(1,i), pcm(2,i), pcm(3,i), pcm(4,i))
          ...
          enddo
          v=pcm(3,i)/pcm(4,i) 
          Y3(i)=0.5 * log ((1+v)/(1-v))
          ylng(i)=ylng(i)+Y3(i)-Y2  !should be equal Y3-Y1
        enddo
...
c rescale all momenta to conserve initial energy
        ...
        do ipass=1,200
          ...
          do  j=1,np
            k1=3
c            pt2=(pcm(1,j)**2+pcm(2,j)**2)
c            scal=1.+(scal-1.)/max(1.,pt2)       !large pt are peaked around eta=0
c            if(pcm(3,j)**2.gt.amass(j)**2/pt2)k1=3
     ...
     *           +amass(j)**2)
          ...
          if(abs(scal-1.).le.errlim) goto 301
        enddo
 301    continue
        do j=1,4
          pa(j)=0.d0
        ...
        do i=1,np
          ...
          v=pcm(3,i)/pcm(4,i) 
          Y4=0.5 * log ((1+v)/(1-v))
          ylng(i)=ylng(i)+Y4-Y3(i)  !should be equal Y4-Y1
        enddo
        pa(5)=sqrt(pa(4)**2-pa(3)**2-pa(2)**2-pa(1)**2)
...
c         bet=bet+pcm(4,i) !would be the correct Lorentz trafo
          aemass=max(sqrt(pt2),amass(i)) !sqrt(pt2+amass(i)**2) !max(sqrt(pt2),amass(i)) !max(0.,amass(i)-0.2)**0.0002
 ...
 300    continue
    ...
    !~~~~~~~~end radial flow stuff ~~~~~~~~~~~~~~~~~~~~~~~~
     ...
     .  ,sngl(p(1)),sngl(p(2)),sngl(p(3)),sngl(p(4)),sngl(p(5)))
        if(jcase.eq.9)then               !ckw
          call getityptl(ip,ityip)
          call setityptl(nptl,ityip+1)
        else !if(yrmax.gt.0.d0)then         !ckw
        ...
        if(jcase.eq.6)then !rope
          call setrinptl(nptl,ylng(n))
        endif 
...
c check
      ...
      ELSE
        am=0.d0    ! problems!!!
      END IF
      if(abs(sngl(am)-tecmor)/tecmor.gt.1e-1)
...
c     --------------------------------------------------------------
      ...
      if(ielast.eq.1)then
        ...
        !~~~~~34~~~~~ 
    2   call hnbpad(2,n1,n2,n3,n4,jc34old)
...
c     --------------------------------
      ...
      if(mod(itot,3).ne.0.or.iatot.gt.6)then
...
#include "aaa.h"
     ...
     .       * ( 3 * T * bessk(2,am/T) + am * bessk1(am/T) )
        enddo
        end
...
#include "aaa.h"
     ...
     *,1*3/
      ...
      if(ifls.ge.1)then
        fac=fac*(1.+iflb*factb-facts)
      else
        fac=fac*(1.+iflb*factb)
      endif
c        if(abs(id).gt.1000)then
c          fac=fac*(1+facts)
c        else!if(abs(id).lt.1000)then
c          fac=fac*(1-facts)
c        endif
c      elseif(abs(id).gt.1000)then
c        fac=fac*(1+factb)
c      endif
c      if(abs(id).gt.1000)fac=fac*(1+factb)
c      if(ifls.ge.1)fac=fac*(1-facts)
      !-----------------
